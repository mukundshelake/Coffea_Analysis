...
class NanoProcessor(processor.ProcessorABC):
    def __init__(self):
        ...
        self.eleTightID_file = 'SFs/UL2016postVFP_ele_ID_SFs.json'
...
#                     print("%^%^%^%^%^*&^%^%^$#%#%")
                ...
                if region=="Total":
                        ...
                        IDsf = ak.unflatten(
                            elIDeval["UL-Electron-ID-SF"].evaluate("2016postVFP","sf","Medium",sel.eta, sel.pt),
                            counts= ak.num(sel, axis=-1))[0,:]
                        IDsfup = ak.unflatten(
                            elIDeval["UL-Electron-ID-SF"].evaluate("2016postVFP","sfup","Medium",sel.eta, sel.pt),
                            counts= ak.num(sel, axis=-1))[0,:]
                        IDsfdown = ak.unflatten(
                            elIDeval["UL-Electron-ID-SF"].evaluate("2016postVFP","sfdown","Medium",sel.eta, sel.pt),
                            counts= ak.num(sel, axis=-1))[0,:]  
