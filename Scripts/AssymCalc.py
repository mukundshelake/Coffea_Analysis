...
class NanoProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        selection.add("leadPtandEta",ak.sum((events.Electron.pt >= 35.0) & (abs(events.Electron.eta) <= 3.0), axis=1) > 0)
        selection.add("atleastThreeJ", ak.num(events.Jet) > 2)
        selection.add("JetPtandEta", ak.sum((events.Jet.pt >= 20.0) & (abs(events.Jet.eta) < 3.0), axis = 1) > 2)
        selection.add("HLTEle32", events.HLT.Ele32_eta2p1_WPTight_Gsf)
        selection.add("ttbarEvent",(events.GenPart.pdgId[:, 2] == 6) & (events.GenPart.pdgId[:, 3] == -6))
        selection.add("uubarEvent", (events.Generator.id1 == 2) & (events.Generator.id2 == -2))
        selection.add("ddbarEvent", (events.Generator.id1 == 1) & (events.Generator.id2 == -1))
        selection.add("ubaruEvent", (events.Generator.id1 == -2) & (events.Generator.id2 == 2))
        selection.add("dbardEvent", (events.Generator.id1 == -1) & (events.Generator.id2 == 1))
        selection.add("higherx1", events.Generator.x1 > events.Generator.x2)
        selection.add("higherx2", events.Generator.x2 > events.Generator.x1)
        selectionList = {
        "ttbar":{'atleastOnelep': True, 'leadPtandEta': True, "atleastThreeJ": True, "JetPtandEta": True, "HLTEle32": True, "ttbarEvent": True},
