...
class NanoProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        event_level = ak.fill_none(req_jet & req_el & req_trig, False)
        if len(events) == 0:
        ...
        if isRealData:
            print(f"working on data {dataset}: No. of events: {sum(event_level)}")
            ...
            if sum(event_level)> 0:
                if hasattr(events, "LHEWeight"):
                    weights.add("LHEWtSign", events[event_level].LHEWeight.originalXWGTUP/abs(events[event_level].LHEWeight.originalXWGTUP))
                # else:
                #     weights.add("LHEWtSign", weight = np.ones(len(events[event_level])))
                    weights.add("puWeight",weight=events[event_level].puWeight)
...
# Log the stuff
metaTxt = "Run: Trying Pruthvis thing for 2018: Remove PuWt up/down variations; also avoiding QCD for filling wts. bcz many QCD chunks have 0 events in selsction  (one valid el and 3 jets, pT , eta cuts, HLT)"
