...
class NanoProcessor(processor.ProcessorABC):
    def __init__(self):
        ...
        # Tight ID configs
        self.muTightID_file = 'SFs/UL2016preVFP_mu_ID_SFs.json'
        
        # HLT configs
        self.muHLT_file = 'SFs/UL2016preVFP_muon_triggers.json'
        
        # B-Tagging configs
        self.btagThreshold = 0.2598
        self.btagSF_file = 'SFs/UL2016preVFP_btagging.json'
        pass
...
#                     print("%^%^%^%^%^*&^%^%^$#%#%")
                ...
                if region=="Total":
                        ...
                        IDsf = ak.unflatten(IDeval["NUM_TightID_DEN_genTracks"].evaluate("2016preVFP_UL", abs(smu.eta), smu.pt, 'sf'),counts = ak.num(smu, axis=0))[0,:]
                        IDsfup = ak.unflatten(IDeval["NUM_TightID_DEN_genTracks"].evaluate("2016preVFP_UL", abs(smu.eta), smu.pt, 'systup'),counts = ak.num(smu, axis=0))[0,:] 
                        IDsfdown = ak.unflatten(IDeval["NUM_TightID_DEN_genTracks"].evaluate("2016preVFP_UL", abs(smu.eta), smu.pt, 'systdown'),counts = ak.num(smu, axis=0))[0,:] 
