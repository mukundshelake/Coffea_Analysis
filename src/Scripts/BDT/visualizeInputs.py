...
def main():
    ...
    logging.info(f"Events Density: {len(df)}")
    ...
    if 'ttbar_mass' in df.columns:
        plt.hist(df[df['y'] == 2]['ttbar_mass'], bins=75, color='blue', label='qg', density=True, histtype='step')
        plt.hist(df[df['y'] == 1]['ttbar_mass'], bins=75, color = 'black', label='gg', density=True, histtype='step')
        plt.hist(df[df['y'] == 0]['ttbar_mass'], bins=75, color = 'red', label='qq', density=True, histtype='step')
