...
def main():
    ...
    parquet_files = [f for f in os.listdir(output_dir) if f.endswith('.parquet') and f.startswith('events_')]
    logging.info(f"Reading {len(parquet_files)} parquet files...")

    df = pd.concat([pd.read_parquet(os.path.join(output_dir, f)) for f in parquet_files], ignore_index=True)
    ...
    grid_search = GridSearchCV(estimator=xgb_binary, param_grid=param_grid, cv=3, scoring='roc_auc', n_jobs=-1)
