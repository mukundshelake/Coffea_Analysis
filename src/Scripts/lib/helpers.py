...
def isValidRootFile(fname):
    fname = os.path.expandvars(fname)  # Expand environment variables in path
    if not os.path.exists(fname):
        return False
    try:
        with uproot.open(fname) as f:
            # Attempt to access a key to ensure the file is not only openable but also readable
            # This is a lightweight way to check if the file seems like a valid ROOT file
            keys = f.keys()
            if len(keys) == 0:  # Check if there are no keys, indicating an empty or invalid file
                return False
    except Exception as e:  # Catch exceptions raised by uproot when opening/reading the file
        return False
    return True
