...
def main():
    ...
    logging.info(f"Number of events in class 0: {len(df[df['y'] == 0])}")
    logging.info(f"Number of events in class 2: {len(df[df['y'] == 2])}")


    # Extract 500000 events from each class
    df_0_sampled = df[df['y'] == 0].sample(n=500000, random_state=42, replace=True)
    df_2_sampled = df[df['y'] == 2].sample(n=500000, random_state=42, replace=True)

    # Combine the sampled data
    df_sampled = pd.concat([df_0_sampled, df_2_sampled], ignore_index=True)
    ...
    logging.info(f"Number of events in training data for class 0: {len(df_train[df_train['y'] == 0])}")
