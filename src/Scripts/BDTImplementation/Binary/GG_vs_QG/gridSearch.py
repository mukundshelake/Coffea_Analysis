...
def main():
    ...
    logging.info(f"Number of events in class 1: {len(df[df['y'] == 1])}")
    logging.info(f"Number of events in class 2: {len(df[df['y'] == 2])}")

    # Filter the dataset to include only events where y is 1 or 2
    df = df[df['y'].isin([1, 2])]

    # Extract 500000 events from each class
    df_1_sampled = df[df['y'] == 1].sample(n=500000, random_state=42, replace=True)
    df_2_sampled = df[df['y'] == 2].sample(n=500000, random_state=42, replace=True)

    # Combine the sampled data
    df_sampled = pd.concat([df_1_sampled, df_2_sampled], ignore_index=True)

    # Split the data into 30% for testing and 70% for training
    df_train, df_test = train_test_split(df_sampled, test_size=0.3, random_state=42, stratify=df_sampled['y'])

    logging.info(f"Number of events in training data for class 1: {len(df_train[df_train['y'] == 1])}")
    logging.info(f"Number of events in training data for class 2: {len(df_train[df_train['y'] == 2])}")
    ...
    # Plot ROC curve
    fpr, tpr, _ = roc_curve(y_test_binary, y_pred_proba_binary, pos_label=2)
    plt.figure()
